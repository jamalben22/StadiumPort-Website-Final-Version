...
function AccordionItem({ question, answer, isOpen, onClick }: { question: string, answer: React.ReactNode, isOpen: boolean, onClick: () => void }) {
 ...
 <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isOpen ? 'bg-amber-500 text-white rotate-180' : ' text-slate-500 group-hover:bg-amber-500/10 group-hover:text-amber-600'}`}>
...
export default function GroupJClientPage() {
  ...
  const AffiliateButton = ({ href, text, icon: Icon = ArrowRight, variant = 'primary' }: { href: string, text: string, icon?: any, variant?: 'primary' | 'secondary' | 'outline' }) => {
    ...
    const variants = {
      primary: "bg-amber-600 text-white hover:bg-amber-500 shadow-[0_10px_40px_-10px_rgba(245,158,11,0.5)] hover:shadow-[0_20px_40px_-10px_rgba(245,158,11,0.6)]",
 ...
 <div className="inline-flex items-center gap-2 py-2 px-4 rounded-full border border-slate-200 dark:border-white/10 shadow-sm backdrop-blur-sm text-amber-700 dark:text-amber-400 text-xs font-bold tracking-[0.15em] uppercase mb-8 animate-fade-in">
 ...
 <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 leading-relaxed">Get our free checklist for Group J travel.</p>
                <AffiliateButton 
                  href="/planner" 
                  text="Start Planner" 
                  variant="secondary"
 ...
 {/* Dallas */}
 <div className="group p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 hover:border-amber-500/30 dark:hover:border-amber-500/30 transition-all duration-500 hover:shadow-xl hover:shadow-amber-900/5">
 ...
 <div className="group text-center p-6 border border-slate-100 dark:border-slate-800 rounded-[2rem] hover:border-amber-500/50 transition-all duration-500 hover:shadow-xl hover:shadow-amber-900/5">
 ...
 <Link href="/planner" className="inline-flex items-center justify-center bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold py-4 px-8 rounded-2xl transition-all hover:scale-105 text-base">
 Start My Frontier Plan
 </Link>
 </div>
 </div>
 </div>
